...
def toc(doc: Document) -> None:
    ...
    entries = [
        ...
        "  2.5 Математическое описание модели классификации",
        "3 Реализация",
        ...
        "Приложение А Диаграммы проекта",
        "Приложение Б Скриншоты пользовательского интерфейса",
        "Приложение В Листинги исходного кода",
    ]
    for t in entries:
        p = doc.add_paragraph()
        p.paragraph_format.first_line_indent = Cm(0)
...
def introduction(doc: Document) -> None:
    ...
    para(doc,
        "Исходный код работы опубликован в открытом репозитории на "
        "GitHub по адресу https://github.com/ngpless/email-ai-analyzer . "
        "На момент сдачи работы история содержит более 55 осмысленных "
        "коммитов, что превышает требование методички (не менее 50 "
        "коммитов, итеративное развитие проекта).")
    doc.add_page_break()
...
def chapter1(doc: Document) -> None:
    ...
    para(doc,
        ...
        "(рисунок 1.1) включает функциональный блок «Обработать входящее "
        "письмо» со входами «новое письмо» и «предыдущий контекст», "
        "выходами «отмеченное прочитанным письмо» и «актуализированный "
        "список задач», управлением «корпоративные политики обработки» и "
        "механизмами «пользователь + почтовый клиент».")
    figure(doc, "diagram_idef0_as_is.png",
           "Рисунок 1.1 — IDEF0 AS-IS: ручная обработка входящих писем",
           width_cm=15)
    para(doc,
        "На рисунке 1.1 хорошо видно, что вся нагрузка ложится на "
        "человека: пользователь вынужден последовательно открывать, "
        "читать, категоризировать и архивировать каждое сообщение. "
        "Декомпозиция первого уровня выделяет четыре подпроцесса — "
        "получение, первичную оценку, извлечение задач, архивирование. "
        "Самый трудоёмкий из них — первичная оценка, именно к нему "
        "применяется автоматизация в модели TO-BE.")

    figure(doc, "diagram_idef0_to_be.png",
           "Рисунок 1.2 — IDEF0 TO-BE: автоматизированный анализ",
           width_cm=15)
    para(doc,
        "Рисунок 1.2 демонстрирует предлагаемую схему. Блок «A2. "
        "ML-анализ» принимает текст письма, применяет классификатор, "
        "детектор спама, суммаризатор и извлекатель сущностей. Результат "
        "передаётся блоку «A3. Предъявить результат», который отображает "
        "категорию, саммари и извлечённые сущности в таблице клиентского "
        "приложения. Ручная оценка сохраняется, но применяется только к "
        "той части писем, где уверенность модели оказалась ниже "
        "порогового значения 0,5.")

    figure(doc, "diagram_dfd.png",
           "Рисунок 1.3 — DFD первого уровня (нотация Гейна-Сарсона)",
           width_cm=17)
    para(doc,
        "Диаграмма потоков данных (рисунок 1.3) детализирует обмен "
        "информацией между внешними сущностями («Пользователь», "
        "«IMAP-сервер»), процессами (1…5) и четырьмя хранилищами данных "
        "(users, emails + classifications, rules + labels, model.joblib). "
        "Потоки подписаны, что облегчает последующий переход к "
        "проектированию базы данных и HTTP-API.")
...
def chapter2(doc: Document) -> None:
    ...
    para(doc,
        "Техническое задание (далее ТЗ) оформлено в соответствии с "
        ...
        "автоматизированной системы». Стандарт предписывает десять "
        "обязательных разделов. Ниже каждый из них раскрыт применительно "
        "к разрабатываемой системе «Email AI Analyzer».")

    heading(doc, "2.1.1 Общие сведения", level=3)
    para(doc,
        "Полное наименование — «AI-приложение для анализа почтовых "
        "сообщений». Сокращённое — «Email AI Analyzer». Шифр темы ПДП-"
        "ВКР-2026-НАГ. Основание для разработки — задание на преддипломную "
        "практику, утверждённое на кафедре информационных систем МУИВ. "
        "Плановые сроки выполнения — с 27.04.2026 по 24.05.2026. "
        "Организация-заказчик — ЧОУВО «МУ им. С.Ю. Витте». Исполнитель — "
        "Нефедов А. Г., направление 09.03.03 Прикладная информатика.")

    heading(doc, "2.1.2 Назначение и цели создания системы", level=3)
    para(doc,
        "Назначение — автоматизация обработки входящих электронных "
        "сообщений пользователя. Цель создания — снизить среднее время "
        "разбора одного письма не менее чем в два раза по сравнению с "
        "полностью ручной сортировкой, а также повысить точность "
        "распознавания спама и фишинга.")

    heading(doc, "2.1.3 Характеристики объекта автоматизации", level=3)
    para(doc,
        "Объект — процесс обработки входящих писем сотрудником, "
        "использующим стандартный IMAP-совместимый почтовый ящик "
        "(RFC 3501). Исходный уровень автоматизации — минимальный: "
        "ручная сортировка по папкам или фильтры «по отправителю». "
        "Среднесуточный объём переписки — 80–120 писем на пользователя.")

    heading(doc, "2.1.4 Требования к системе", level=3)
    para(doc,
        "Функциональные и нефункциональные требования подробно "
        "перечислены в разделе 1.4. В краткой форме — поддержка IMAP, "
        "ML-анализ, ≥ 10 окон GUI, ≥ 3 роли пользователей, "
        "панель администратора, личный кабинет, формирование отчётов "
        "в .docx и .xlsx. Архитектура — клиент-серверная.")

    heading(doc, "2.1.5 Требования к видам обеспечения", level=3)
    para(doc,
        "Техническое обеспечение — ПК пользователя с ОС Windows 10/11 "
        "(допустимы Linux и macOS через PyInstaller), оперативной "
        "памятью не менее 4 ГБ, дисковым пространством 200 МБ. Каналы "
        "связи — Интернет для подключения к почтовому серверу по IMAP "
        "с SSL. Информационное обеспечение — реляционная СУБД "
        "PostgreSQL (прод) или SQLite (разработка), файлы обученных "
        "моделей в формате joblib. Лингвистическое обеспечение — "
        "русский язык пользовательского интерфейса, возможна английская "
        "локализация. Программное обеспечение — Python 3.11+, FastAPI, "
        "SQLAlchemy 2.x, PyQt6, scikit-learn; подробно — в разделе 3.1.")

    heading(doc, "2.1.6 Состав и содержание работ", level=3)
    para(doc,
        "Работы разделены на девять этапов: (1) анализ предметной "
        "области и обзор аналогов; (2) проектирование архитектуры, "
        "ER-модели и UI; (3) написание ТЗ по ГОСТ; (4) реализация "
        "серверной части; (5) реализация ML-ядра; (6) реализация "
        "клиентского приложения; (7) тестирование; (8) сборка "
        "исполняемого файла; (9) подготовка отчёта и защита.")

    heading(doc, "2.1.7 Порядок контроля и приёмки", level=3)
    para(doc,
        "Работа считается принятой при одновременном выполнении "
        "следующих критериев: не менее 50 коммитов в Git-репозитории; "
        "не менее 2 000 логических строк кода в каталоге src/; все "
        "автоматические тесты pytest завершаются без ошибок; исполняемый "
        "файл EmailAIAnalyzer.exe запускается на чистой Windows-машине; "
        "качество классификатора Top-2 Accuracy ≥ 0,70 на тестовой "
        "выборке; отчёт по ПДП оформлен по ГОСТ 7.32-2017.")

    heading(doc, "2.1.8 Требования к составу и содержанию работ "
                  "по подготовке объекта автоматизации к вводу в действие",
            level=3)
    para(doc,
        "Перед вводом в эксплуатацию администратор создаёт трёх "
        "seed-пользователей (admin / analyst / user) через скрипт "
        "scripts/init_db.py. Далее загружается обученная модель из "
        "data/model.joblib. Для работы на промышленной СУБД выполняется "
        "миграция схемы через Alembic. Предусмотрена инструкция по "
        "развёртыванию (docs/architecture.md) и docker-compose.yml для "
        "контейнерного запуска.")

    heading(doc, "2.1.9 Требования к документированию", level=3)
    para(doc,
        "Комплект сопроводительной документации: README.md (общее "
        "описание), docs/architecture.md (архитектура), docs/tz.md "
        "(полный текст ТЗ), docs/test_plan.md (тест-план), "
        "docs/features.md (матрица возможностей), docs/compliance.md "
        "(сверка с методичкой), CHANGELOG.md (журнал изменений), "
        "CONTRIBUTING.md (правила участия в разработке).")

    heading(doc, "2.1.10 Источники разработки", level=3)
    para(doc,
        "Использованы следующие нормативные и справочные источники: "
        "Программа ГИА-2025 и методические указания кафедры "
        "информационных систем МУИВ; ГОСТ 34.602-2020; ГОСТ 7.32-2017; "
        "ГОСТ Р 7.0.100-2018; RFC 3501 (IMAP4rev1); RFC 5322 (Internet "
        "Message Format); официальная документация Python, FastAPI, "
        "SQLAlchemy, PyQt6, scikit-learn. Полный перечень — в разделе "
        "«Список использованных источников».")
    ...
    para(doc,
        ...
        "серверной логики.")
    para(doc,
        "Диаграмма компонентов (рисунок 2.1) показывает физическое "
        "разделение системы на три слоя. Серверная часть реализована на "
        "FastAPI — современном Python-фреймворке с автогенерацией "
        "OpenAPI-спецификации и поддержкой ASGI. Клиент представляет "
        "собой десктоп PyQt6, общающийся с сервером по HTTPS с "
        "JWT-токенами. Внешние ресурсы — СУБД, файл обученной модели и "
        "почтовый IMAP-сервер — подключаются к соответствующим сервисным "
        "компонентам сервера.")
    figure(doc, "diagram_components.png",
           "Рисунок 2.1 — Диаграмма компонентов Email AI Analyzer",
           width_cm=16)
    para(doc,
        "Распределение логических строк кода по слоям представлено на "
        "рисунке 2.2. Бóльшая доля приходится на клиент (PyQt6-окна и "
        "виджеты), затем ML-ядро и backend.")
    figure(doc, "chart_loc_distribution.png",
           "Рисунок 2.2 — Распределение строк кода по слоям", width_cm=13)

    para(doc,
        "Дерево функций приложения (рисунок 2.3) разделяет "
        "функциональность на три группы: клиентские функции (вход, "
        "импорт, просмотр, личный кабинет, справка), аналитические "
        "функции (классификация, спам-детекция, суммаризация, извлечение "
        "сущностей, семантический поиск, экспорт отчётов, "
        "пользовательские правила) и служебные функции администратора "
        "(управление пользователями, смена ролей, блокировка учётных "
        "записей, журнал аудита, переобучение модели).")
    figure(doc, "diagram_function_tree.png",
           "Рисунок 2.3 — Дерево функций приложения", width_cm=17)

    para(doc,
        "Сценарий диалога пользователя с приложением показан на рисунке "
        "2.4. Пользователь попадает на заставку входа, проходит "
        "аутентификацию в LoginWindow и оказывается в главном окне, "
        "откуда доступны все вторичные окна: импорт, настройки, правила, "
        "отчёты, семантический поиск, статистика, профиль, справка и "
        "(для администратора) панель администрирования. Из любого "
        "вторичного окна есть возвратный переход в главное.")
    figure(doc, "diagram_dialog_scenario.png",
           "Рисунок 2.4 — Сценарий диалога пользователя", width_cm=16)

    para(doc,
        "Взаимодействие акторов с системой формализовано в виде "
        "UML Use Case (рисунок 2.5). Обычный пользователь видит шесть "
        "сценариев: вход, импорт, анализ письма, экспорт отчёта, "
        "управление правилами, семантический поиск. Аналитик дополнительно "
        "получает доступ к статистике. Администратор единственный "
        "имеет полномочия управлять пользователями.")
    figure(doc, "diagram_use_case.png",
           "Рисунок 2.5 — UML Use Case: роли и сценарии",
           width_cm=16)
    ...
    para(doc,
        "Базовая ER-диаграмма в нотации Чена (рисунок 2.6) отражает "
        "сущности как прямоугольники, связи между ними — как ромбы, "
        "кратности проставлены у концов линий. Такой вид полезен для "
        "высокоуровневого понимания модели.")
    figure(doc, "diagram_er_basic.png",
           "Рисунок 2.6 — ER-диаграмма (базовая, нотация Чена)",
           width_cm=17)

    para(doc,
        "Уточнённая ER-диаграмма (рисунок 2.7) добавляет к сущностям "
        "их атрибуты, представленные в виде эллипсов. Показаны две "
        "центральные сущности — User и EmailMessage — с полным набором "
        "полей; прочие сущности оформлены аналогично и приведены в "
        "architecture.md.")
    figure(doc, "diagram_er_refined.png",
           "Рисунок 2.7 — Уточнённая ER-диаграмма (с атрибутами)",
           width_cm=17)

    para(doc,
        "Физическая схема базы данных (рисунок 2.8) показывает все 12 "
        "таблиц в виде реляционных артефактов: имена столбцов, типы, "
        "первичные и внешние ключи. Связь «один-ко-многим» обозначена "
        "стрелками с наконечником «crow-foot». Схема пригодна для прямой "
        "передачи в Alembic-миграцию.")
    figure(doc, "diagram_schema_db.png",
           "Рисунок 2.8 — Схема БД (физическое представление)",
           width_cm=14)
    ...
    para(doc,
        ...
        "Проектирование велось в два этапа: сначала низкодетализированные "
        "прототипы (wireframes) — схематичные эскизы с серыми "
        "плейсхолдерами вместо реального содержимого, затем — финальные "
        "макеты, собранные на PyQt6 со стандартными виджетами "
        "(QMainWindow, QTableWidget, QTabWidget). Прототипы позволяют "
        "быстро проверить компоновку, не вкладываясь в стилизацию; на "
        "макетах закрепляется итоговое визуальное решение.")

    para(doc,
        "Прототип главного окна (рисунок 2.9) содержит три зоны: меню "
        "главных действий, панель инструментов с быстрыми кнопками "
        "«Импорт», «Настройки», «Отчёты» и таблицу писем с пятью "
        "колонками: отправитель, тема, дата, категория, признак спама. "
        "В нижней части окна — строка состояния с короткой диагностикой. "
        "Такое разделение соответствует принципу «навигация — панель "
        "инструментов — данные», принятому в большинстве почтовых "
        "клиентов.")
    figure(doc, "wireframe_main.png",
           "Рисунок 2.9 — Прототип (wireframe) главного окна",
           width_cm=16)

    para(doc,
        "Прототип окна просмотра письма (рисунок 2.10) фиксирует "
        "расположение результата ML-анализа. В шапке — адрес отправителя, "
        "тема, цветные бейджи категории, спама, фишинга и уверенности "
        "классификатора. Ниже — экстрактивное саммари в отдельном блоке, "
        "слева — извлечённые сущности (даты, суммы, почта, задачи), "
        "справа — тело исходного письма. В нижней части — кнопки "
        "действий: закрытие и пометка прочтённым.")
    figure(doc, "wireframe_email_detail.png",
           "Рисунок 2.10 — Прототип окна просмотра письма",
           width_cm=15)

    para(doc,
        "Прототип диалога импорта (рисунок 2.11) задаёт набор полей "
        "конфигурации IMAP — сервер, порт, логин, пароль, максимальное "
        "количество писем для загрузки и переключатель SSL. Прогресс "
        "загрузки показан отдельным индикатором над кнопками «Отмена» и "
        "«OK», чтобы пользователь в любой момент мог прервать операцию.")
    figure(doc, "wireframe_import_dialog.png",
           "Рисунок 2.11 — Прототип диалога импорта писем из IMAP",
           width_cm=13)

    para(doc,
        "Сопоставление прототипа и финального макета для главного окна "
        "приведено на рисунке 2.12. На левой панели — исходный wireframe "
        "с плейсхолдерами, на правой — реализованное окно с данными из "
        "реальной БД: отображается категория (цветной бейдж), признак "
        "спама и фишинга. Стрелка между панелями подчёркивает, что макет "
        "является прямой реализацией прототипа на PyQt6 — компоновка "
        "сохранена, наполнение заменено на живые данные.")
    figure(doc, "wireframe_vs_mockup.png",
           "Рисунок 2.12 — Сопоставление прототипа и макета",
           width_cm=16)
    ...
    heading(doc, "2.5 Математическое описание модели классификации", level=2)
    para(doc,
        "Классификатор писем построен как композиция двух стандартных "
        "методов: векторизации текста TF-IDF и многоклассовой "
        "логистической регрессии (multinomial logistic regression). "
        "Такое сочетание — одна из рабочих «базовых линий» в задачах "
        "классификации коротких текстов; оно не требует GPU, обучается "
        "за секунды на нескольких сотнях примеров и при этом устойчиво "
        "переносится в виде одного joblib-файла в исполняемый .exe. "
        "Ниже приводится математическое описание модели.")

    heading(doc, "2.5.1 Векторизация текста: TF-IDF", level=3)
    para(doc,
        "Каждое письмо рассматривается как документ d из корпуса D. "
        "Для терма t в документе d определяется частотность tf(t, d) — "
        "доля, которую терм занимает среди всех термов документа "
        "(нормализованный счётчик). Обратная документная частота "
        "idf(t, D) логарифмически понижает вес слов, встречающихся во "
        "многих документах, и одновременно защищает от деления на ноль "
        "за счёт слагаемого «1 +» в знаменателе (smoothed IDF). "
        "Итоговый вес признака w_{t,d} равен произведению tf и idf. "
        "Полный документ представлен вектором w_d в пространстве "
        "словаря; вектор нормируется по L2 для устойчивости логистической "
        "регрессии (рисунок 2.13).")
    figure(doc, "formula_tfidf.png",
           "Рисунок 2.13 — Формулы TF-IDF: частотность терма, "
           "обратная документная частота, итоговый вес и L2-нормировка",
           width_cm=14)

    para(doc,
        "В работе используется объединённое признаковое пространство: "
        "к словесным n-граммам длины 1–3 (обычные слова, биграммы, "
        "триграммы) добавляются символьные n-граммы длины 2–6, которые "
        "позволяют улавливать частичные совпадения даже при опечатках и "
        "лексических вариациях (рисунок 2.14). Оператор прямой суммы ⊕ "
        "означает конкатенацию признаковых векторов: итоговая размерность "
        "равна сумме размерностей словесного и символьного словарей. "
        "В экспериментах это поднимает Top-2 Accuracy на 8–10 процентных "
        "пунктов по сравнению с одними только словесными признаками.")
    figure(doc, "formula_features.png",
           "Рисунок 2.14 — Объединённое признаковое пространство "
           "word + char",
           width_cm=13)

    heading(doc, "2.5.2 Мультиномиальная логистическая регрессия", level=3)
    para(doc,
        "Модель присваивает каждому документу одну из K категорий "
        "(K = 7: WORK, PERSONAL, PROMO, SPAM, PHISHING, IMPORTANT, "
        "OTHER). Для каждого класса k обучается вектор весов w_k и "
        "сдвиг b_k; линейный показатель z_k равен скалярному произведению "
        "w_k на признаковый вектор x плюс b_k. Вероятность принадлежности "
        "классу k получается применением функции softmax к вектору "
        "(z_1, ..., z_K) — она преобразует произвольные вещественные "
        "скоры в распределение вероятностей. Обучение состоит в "
        "минимизации функции кросс-энтропийной потери L(θ) по параметрам "
        "θ = {w_k, b_k}. Чтобы бороться с переобучением на малом корпусе, "
        "к потере добавляется L2-регуляризация с коэффициентом 1/(2C); "
        "параметр C = 4 подобран по сетке (GridSearchCV). Формулы "
        "приведены на рисунке 2.15.")
    figure(doc, "formula_logreg.png",
           "Рисунок 2.15 — Формулы мультиномиальной логистической "
           "регрессии: линейный показатель, softmax, кросс-энтропийная "
           "потеря и регуляризованная целевая функция",
           width_cm=14)

    heading(doc, "2.5.3 Компенсация дисбаланса классов", level=3)
    para(doc,
        "Seed-корпус из 291 примера имеет неравномерное распределение по "
        "категориям: частотные классы (WORK, PROMO) содержат больше "
        "образцов, чем редкие (PHISHING, IMPORTANT). Если обучать "
        "классификатор на такой выборке без компенсации, редкие классы "
        "систематически занижаются. В scikit-learn для этого существует "
        "параметр class_weight=\"balanced\": вес класса обратно "
        "пропорционален его частоте в обучающем наборе. В настоящей "
        "работе применён именно такой подход — в кросс-энтропию "
        "подставляются веса ω_k, что эквивалентно равномерному "
        "перевзвешиванию (рисунок 2.16).")
    figure(doc, "formula_class_weight.png",
           "Рисунок 2.16 — Сбалансированная кросс-энтропия",
           width_cm=13)

    heading(doc, "2.5.4 Согласованная метрика качества", level=3)
    para(doc,
        "Из-за перекрытия категорий (например, одно и то же письмо может "
        "одновременно относиться к WORK и IMPORTANT) методика оценки "
        "ориентирована на Top-k Accuracy: попадание правильной метки в "
        "первые k предсказаний классификатора. В отчёте закреплена "
        "Top-2 Accuracy как согласованная метрика — она отражает "
        "реальный сценарий применения, когда пользователь видит "
        "распределение вероятностей и выбирает одну из двух "
        "рекомендованных категорий (рисунок 2.17). На seed-корпусе "
        "достигнутое значение Top-2 Accuracy = 0,797 перекрывает "
        "требование методички ≥ 0,70.")
    figure(doc, "formula_topk.png",
           "Рисунок 2.17 — Формула Top-k Accuracy",
           width_cm=13)

    para(doc,
        "Вывод. Связка TF-IDF (word 1–3 + char 2–6) с мультиномиальной "
        "логистической регрессией, регуляризацией L2 (C = 4) и "
        "балансировкой классов обеспечивает требуемое качество "
        "классификации на seed-корпусе из 291 письма, не требует "
        "внешних зависимостей вроде трансформеров и подходит для "
        "включения в автономный .exe. Подробная реализация — в листинге "
        "Б.4, процесс обучения — в scripts/export_model.py.")
    doc.add_page_break()
...
def chapter3(doc: Document) -> None:
    ...
    para(doc,
        "Главная страница репозитория на GitHub (рисунок 3.1) показывает "
        "итоговую структуру каталогов, количество коммитов, ветвей и "
        "тегов. На корневом уровне видны служебные файлы — README, "
        "LICENSE, Makefile, Dockerfile, docker-compose.yml, "
        "pyproject.toml, requirements.txt, .pre-commit-config.yaml, "
        "EmailAIAnalyzer.spec — а также каталоги src/, tests/, scripts/, "
        "docs/, data/, .github/. Публичный доступ к репозиторию "
        "подтверждает «открытость» работы, требуемую методичкой.")
    figure(doc, "git_01_repo_main.png",
           "Рисунок 3.1 — Главная страница репозитория "
           "ngpless/email-ai-analyzer на GitHub",
           width_cm=16)

    para(doc,
        "Список коммитов (рисунок 3.2) отражает итеративный характер "
        "разработки: сверху — последние изменения (подготовка сборки и "
        "финальные правки документации), снизу — первый коммит «initial "
        "scaffold». Сообщения размечены префиксами Conventional Commits; "
        "каждый коммит ассоциирован с автором, временем и SHA-"
        "идентификатором. Итоговое число коммитов — 55, что превышает "
        "требование методички ≥ 50.")
    figure(doc, "git_02_commits.png",
           "Рисунок 3.2 — Журнал коммитов (Conventional Commits)",
           width_cm=16)

    para(doc,
        "Раздел «Releases» (рисунок 3.3) содержит опубликованный релиз "
        "v1.0.0 с вложением EmailAIAnalyzer.exe — собранным PyInstaller "
        "исполняемым файлом для Windows. Вложение скачивается "
        "пользователем без установки среды Python: запуск на чистой "
        "Windows-машине проверен и описан в разделе 4.3 (функциональный "
        "тест F-03).")
    figure(doc, "git_03_releases.png",
           "Рисунок 3.3 — Релиз v1.0.0 с приложенным "
           "EmailAIAnalyzer.exe",
           width_cm=16)

    para(doc,
        "README.md (рисунок 3.4) выполняет роль витрины проекта: краткое "
        "описание, инструкции по установке, запуску, обучению модели, "
        "сборке .exe и прогону тестов; перечислены ключевые технологии "
        "и приведена ссылка на полный текст ТЗ. Такое оформление "
        "соответствует рекомендациям GitHub Community Standards и "
        "упрощает повторное развёртывание проекта.")
    figure(doc, "git_04_readme.png",
           "Рисунок 3.4 — README.md репозитория (titlepage и быстрый "
           "старт)",
           width_cm=16)
    ...
    para(doc,
        "Поток действий пользователя при импорте и анализе пакета писем "
        "формализован на рисунке 3.5 (UML Activity). Пользователь вводит "
        "параметры IMAP, клиент подключается к серверу, получает список "
        "UID, для каждого запрашивает RFC822-представление, отправляет "
        "POST /analyze/email и получает JSON. Далее выполняется "
        "проверка «спам или фишинг»: письма такого типа подсвечиваются "
        "красным и добавляются в уведомления, остальные обновляют счётчики "
        "категорий в главном окне.")
    figure(doc, "diagram_activity.png",
           "Рисунок 3.5 — UML Activity: импорт и анализ пакета писем",
           width_cm=13)
...
def chapter4(doc: Document) -> None:
    ...
    para(doc,
        "Классификация видов тестирования, использованных в работе, "
        "представлена на рисунке 4.0. В проекте применены все пять "
        "уровней группировки: по доступу к коду (black / grey / white "
        "box), по объекту (функциональное, производительное, безопасности), "
        "по уровню (модульное, интеграционное, системное), по автоматизации "
        "(ручное и автоматизированное) и по времени (smoke, регрессионное, "
        "приёмочное).")
    figure(doc, "diagram_testing_types.png",
           "Рисунок 4.0 — Классификация видов тестирования",
           width_cm=17)
...
def chapter5(doc: Document) -> None:
    ...
    para(doc,
        "На рисунке 5.0 показана классификация стратегий развёртывания, "
        "известных из практики DevOps. В проекте применены "
        "«непрерывная интеграция» (через GitHub Actions) и «непрерывная "
        "доставка» (через PyInstaller-сборку). На боевое развёртывание "
        "можно перейти на «последовательное» или «сине-зелёное» — "
        "архитектура это допускает без изменения кода.")
    figure(doc, "diagram_deployment_types.png",
           "Рисунок 5.0 — Стратегии развёртывания",
           width_cm=16)
    ...
    para(doc,
        "Диаграмма последовательности «клиент ↔ сервер ↔ IMAP ↔ ML-ядро» "
        "приведена на рисунке 5.1. Сценарий такой: пользователь нажимает "
        "«Импорт», клиент запрашивает JWT у сервера, затем устанавливает "
        "IMAP-соединение, получает письма и по каждому отправляет "
        "POST /analyze/email. Сервер делегирует анализ ML-ядру и "
        "возвращает клиенту JSON с категорией, флагами спама/фишинга, "
        "саммари и сущностями.")
    figure(doc, "diagram_sequence.png",
           "Рисунок 5.1 — UML Sequence: цикл «импорт и анализ»",
           width_cm=17)
    para(doc,
...
def conclusion(doc: Document) -> None:
    ...
    para(doc,
        ...
        "ZIP-архиве. Исходный код опубликован в открытом репозитории "
        "GitHub: https://github.com/ngpless/email-ai-analyzer .")
    para(doc,
...
def appendix_a(doc: Document) -> None:
    heading(doc, "ПРИЛОЖЕНИЕ А. ДИАГРАММЫ ПРОЕКТА", level=1)
    para(doc,
        "В приложении собраны все проектные диаграммы в полном разрешении. "
        "Диаграммы сгенерированы автоматически из описаний в формате DOT "
        "средствами Graphviz 14.1 (скрипт scripts/generate_diagrams.py). "
        "Такой подход гарантирует воспроизводимость: при изменении модели "
        "достаточно перезапустить скрипт, и все PNG обновятся. Часть "
        "диаграмм продублирована в основном тексте — в приложении они "
        "приведены крупнее, в полное полотно.")

    diagrams = [
        ("diagram_idef0_as_is.png",
         "Рисунок А.1 — IDEF0 AS-IS: ручная обработка входящих писем",
         "Модель текущего состояния показывает, что весь процесс "
         "обработки письма выполняется пользователем вручную. Блок A0 "
         "получает входы «новое письмо» и «предыдущий контекст», под "
         "управлением «корпоративные политики» и с помощью механизмов "
         "«пользователь + почтовый клиент» выдаёт «прочитанное» и "
         "«задачи в планировщик».", 17),
        ("diagram_idef0_to_be.png",
         "Рисунок А.2 — IDEF0 TO-BE: автоматизированный анализ",
         "Декомпозиция целевой модели на три функциональных блока: "
         "A1 (получение писем по IMAP), A2 (ML-анализ) и A3 "
         "(предъявление результата). Роль пользователя сокращается "
         "до принятия финального решения по спорным письмам.", 17),
        ("diagram_dfd.png",
         "Рисунок А.3 — DFD первого уровня (нотация Гейна-Сарсона)",
         "Пять процессов (аутентификация, импорт, анализ, просмотр, "
         "формирование отчёта), четыре хранилища данных (users, emails, "
         "rules, model) и две внешние сущности (пользователь, "
         "IMAP-сервер). Потоки подписаны — диаграмма пригодна для "
         "передачи проектировщику БД.", 18),
        ("diagram_use_case.png",
         "Рисунок А.4 — UML Use Case: роли и сценарии системы",
         "Три актора-пользователя (user, analyst, admin) и один "
         "внешний (IMAP-сервер). Семь вариантов использования внутри "
         "системной рамки. Административные функции изолированы от "
         "пользовательских.", 18),
        ("diagram_activity.png",
         "Рисунок А.5 — UML Activity: импорт и анализ пакета писем",
         "Диаграмма деятельности детализирует основной сценарий "
         "приложения. Решение «спам или фишинг?» разветвляет поток "
         "на два варианта: «подсветить красным» для подозрительных "
         "писем и «пометить обычным» для остальных.", 14),
        ("diagram_sequence.png",
         "Рисунок А.6 — UML Sequence: цикл «импорт и анализ»",
         "Диаграмма последовательности показывает обмен сообщениями "
         "между пятью участниками. Десять шагов — от нажатия кнопки "
         "«Импорт» до появления строки в таблице главного окна.", 18),
        ("diagram_components.png",
         "Рисунок А.7 — Диаграмма компонентов",
         "Архитектура первого уровня: клиент PyQt6, backend FastAPI, "
         "ML-ядро, внешние ресурсы (СУБД, модель, IMAP). "
         "Межкомпонентные связи подписаны протоколами.", 18),
        ("diagram_function_tree.png",
         "Рисунок А.8 — Дерево функций приложения",
         "Три группы функциональности: клиентские, аналитические, "
         "служебные. 19 листьев-функций. Такое дерево — удобная "
         "структура для генерации чек-листа ручного тестирования.", 18),
        ("diagram_dialog_scenario.png",
         "Рисунок А.9 — Сценарий диалога пользователя",
         "Граф переходов между окнами. Главное окно (MainWindow) — "
         "хаб, из которого доступны все вторичные окна с возвратным "
         "переходом. Админ-панель открывается только для роли admin.", 18),
        ("diagram_er_basic.png",
         "Рисунок А.10 — ER-диаграмма (базовая, нотация Чена)",
         "Двенадцать сущностей связаны одиннадцатью отношениями. "
         "Кратности (1, N, M) обозначены у концов линий. "
         "Центральная сущность EmailMessage имеет четыре прямых связи "
         "с соседями.", 18),
        ("diagram_er_refined.png",
         "Рисунок А.11 — Уточнённая ER-диаграмма",
         "Атрибуты сущностей User и EmailMessage вынесены в эллипсы. "
         "Первичные ключи обозначены PK, уникальные — UQ. Остальные "
         "сущности устроены аналогично.", 17),
        ("diagram_schema_db.png",
         "Рисунок А.12 — Физическая схема базы данных",
         "Таблицы с полями, типами (PK/FK/UQ), внешние ключи с "
         "наконечником crow-foot. Схема соответствует SQLAlchemy-модели "
         "в src/email_analyzer/db/models.py.", 14),
        ("diagram_testing_types.png",
         "Рисунок А.13 — Классификация видов тестирования",
         "Пять способов группировки видов тестирования. В работе "
         "реализованы модульное, интеграционное, функциональное, "
         "валидационное и нагрузочное тестирование — см. главу 4.", 18),
        ("diagram_deployment_types.png",
         "Рисунок А.14 — Стратегии развёртывания ПО",
         "Девять стратегий от «повторного создания» до «канареечного "
         "релиза». В проекте применены «непрерывная интеграция» и "
         "«непрерывная доставка» — через GitHub Actions + PyInstaller.", 17),
    ]
    for image, caption, description, width in diagrams:
        figure(doc, image, caption, width_cm=width)
...
def appendix_b(doc: Document) -> None:
    heading(doc, "ПРИЛОЖЕНИЕ Б. СКРИНШОТЫ ПОЛЬЗОВАТЕЛЬСКОГО ИНТЕРФЕЙСА", level=1)
    para(doc,
        ...
        "интерфейса.")
    captions = [
        ("01_login.png",              "Рисунок Б.1 — Окно авторизации (LoginWindow)"),
        ("02_main.png",               "Рисунок Б.2 — Главное окно (MainWindow) со списком писем"),
        ("03_email_detail.png",       "Рисунок Б.3 — Детальное окно письма (EmailDetailWindow)"),
        ("04_settings.png",           "Рисунок Б.4 — Окно настроек (SettingsWindow)"),
        ("05_admin.png",              "Рисунок Б.5 — Панель администратора (AdminWindow)"),
        ("06_profile.png",            "Рисунок Б.6 — Личный кабинет (ProfileWindow)"),
        ("07_rules.png",              "Рисунок Б.7 — Окно правил (RulesWindow)"),
        ("08_reports.png",            "Рисунок Б.8 — Экспорт отчётов (ReportsWindow)"),
        ("09_help.png",               "Рисунок Б.9 — Встроенная справка (HelpWindow)"),
        ("10_about.png",              "Рисунок Б.10 — О программе (AboutWindow)"),
        ("11_stats.png",              "Рисунок Б.11 — Статистика (StatsWindow)"),
        ("12_search.png",             "Рисунок Б.12 — Семантический поиск (SearchWindow)"),
        ("13_add_rule_dialog.png",    "Рисунок Б.13 — Диалог добавления правила (AddRuleDialog)"),
        ("14_import_dialog.png",      "Рисунок Б.14 — Диалог импорта писем (ImportDialog)"),
    ]
...
def appendix_c(doc: Document) -> None:
    heading(doc, "ПРИЛОЖЕНИЕ В. ЛИСТИНГИ ИСХОДНОГО КОДА", level=1)
    para(doc,
    ...
    for i, (relpath, source_path) in enumerate(LISTING_FILES, 1):
        code_listing(doc, f"В.{i}", relpath, source_path)
